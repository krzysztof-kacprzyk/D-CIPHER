...
def save_output_no_gp(filename, trial, seed, var_errors, time_elapsed):
    message = f"""
----------------------
Trial: {trial}
Seed: {seed}
nearest_error: {var_errors['nearest']}
linear_error: {var_errors['linear']}
cubic_error: {var_errors['cubic']}
Time elapsed: {time_elapsed}"""

    with open(filename, 'a') as f:
        f.write(message)

def df_append_no_gp(old_df, trial, seed, var_errors, time_elapsed):
    df = pd.DataFrame()
    df['trial'] = [trial]
    df['seed'] = [seed]
    df['nearest_error'] = [var_errors['nearest']]
    df['linear_error'] = [var_errors['linear']]
    df['cubic_error'] = [var_errors['cubic']]
...
if __name__ == '__main__':
    ...
    if not args.no_gp:
        raise ValueError("GP not implemented in these experiments")
    ...
    filename = f"results/run_var_square_interp_{dt}.txt"
    filename_csv = f"results/run_var_square_interp_{dt}_table.csv"
    filename_meta = f"results/run_var_square_interp_{dt}_meta.p"
...
gplearn config: {gp_params}
    ...
    for trial, seed in enumerate(seeds):
        ...
        for interpolation in ['nearest','linear','cubic']:
            ...
            start = time.time()
            full_dataset = estimate_fields(observed_grid,observed_dataset,full_grid,seed=seed, method=interpolation)
            end = time.time()
            ...
            var_errors[interpolation] = np.sqrt(np.mean((best_found_weights - target_weights) ** 2))

        trial_end = time.time()

        save_output_no_gp(filename, trial+1, seed, var_errors, trial_end-trial_start)
        
        df = df_append_no_gp(df,trial+1, seed, var_errors, trial_end-trial_start)
