...
class Model:
    ...
    def __init__(self, integrals_tensor, g_tensor, alpha, beta, num_lower_order, homogeneous):
        self.integrals_tensor = integrals_tensor
    ...
    def loss(self, weights):
        ...
        pred = torch.mul(weights, self.integrals_tensor).sum(-1) - self.g_tensor
...
class LinearOperatorFinder:
    ...
    def __init__(self,estimated_dataset, field_index, full_grid, dimension, order, basis, index_limits, alpha=0.1, beta=0.1, optim_name='sgd', optim_params={'lr':0.01}, num_epochs=100, patience=10,  seed=0):
        ...
        self.integrals_tensor = torch.from_numpy(integrals) # shape: (D,S,J)
    ...
    def find_weights(self, g_function=None):
        ...
        if not homogeneous:
            g_part_func = g_function(np.swapaxes(self.grid_and_fields,0,1)) # should have shape (D,*self.full_grid.shape)
            assert g_part_func.shape == (self.D,*self.full_grid.shape)

            g_part = np.multiply(g_part_func[:,np.newaxis], self.test_function_part[np.newaxis,:,0])
            assert g_part.shape == (self.D,self.S,*self.full_grid.shape)

            g_integrals = np.multiply(g_part, self.full_grid.for_integration()).sum(axis=tuple(range(3, len(g_part.shape))))
            assert g_integrals.shape == (self.D, self.S)
        else:
            g_integrals = np.zeros((self.D, self.S))

        model = Model(self.integrals_tensor,torch.from_numpy(g_integrals),self.alpha,self.beta,self.num_lower_order,homogeneous)

        weights = torch.randn(LinearOperator.get_vector_length(self.dimension, self.order), requires_grad=True)
